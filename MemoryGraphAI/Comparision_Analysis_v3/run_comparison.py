...
#         graph_answer = graph_pipeline.answer_question(query, llm)
#         graph_latency = time.time() - t0

#         graph_rank = compute_ranking_metrics(graph_retrieved, relevant, K_GRAPH, graph_latency)
#         graph_judge = llm_judge_answer(query, ground_truth, graph_answer, llm)
#         graph_ctx_judge = llm_judge_context(query, graph_context, llm)

#         # ── Vector RAG ─────────────────────────────────────────────────────
#         t1 = time.time()
#         vector_retrieved, _ = vector_pipeline.search(query)
#         vector_context = vector_pipeline.get_context_string(query)
#         vector_answer = vector_pipeline.answer_question(query, llm)
#         vector_latency = time.time() - t1

#         vector_rank = compute_ranking_metrics(vector_retrieved, relevant, K_VECTOR, vector_latency)
#         vector_judge = llm_judge_answer(query, ground_truth, vector_answer, llm)
#         vector_ctx_judge = llm_judge_context(query, vector_context, llm)
